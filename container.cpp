...
void Container::save(std::string file_name){
    std::ofstream file;
    ...
    file<<this->iterator->member.name;
    file<<"\n";
    file<<this->iterator->member.membership_id;
    file<<"\n";
    if(iterator->member.is_preferred)
        pref="Preferred";
    else
        pref="Basic";
    file<<pref;
    file<<"\n";
    temp = this->iterator->member.exp_date;
    temp.insert(2, "/");
    temp.insert(5,"/");
    file<<temp;
    file<<"\n";
